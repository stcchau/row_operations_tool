...
public class Number {
	...
	public Number add(Number n) {
		...
		Number  sum = new Number(numerator * (lcm / denominator) + n.getNumerator() * (lcm / n.getDenominator()), lcm);
		sum.reduceFraction();
		return sum;
	}
	
	public Number multiply(Number n) {
		Number product = new Number(numerator * n.getNumerator(), denominator * n.getDenominator());
		product.reduceFraction();
		return product;
	}
	
	public Number divide(Number n) {
		Number quotient = new Number(numerator * n.getDenominator(), denominator * n.getNumerator());
		quotient.reduceFraction();
		return quotient;
	}

	
	private int getLCM(Number n1, Number n2) {
	...
	public static Number parseNumber(int n) {
