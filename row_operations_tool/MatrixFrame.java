...
public class MatrixFrame extends JFrame implements ActionListener {
	...
	private int cur = -1;
	private final JButton ADD_BUTTON = new JButton("Add Rows"), 
	...
	public void addToRow(int addeeRow, int adderRow, Fraction multiplier) throws Exception {
		fixHistory();
		for(int c = 0; c < COLUMNS; c++) {
			Fraction addee = Fraction.parseFraction(JLMatrix[addeeRow][c].getText());
			Fraction adder = Fraction.parseFraction(JLMatrix[adderRow][c].getText());
			JLMatrix[addeeRow][c].setText(addee.add(adder.multiply(multiplier)).toString());
		}
		addToHistory();
	}
	
	public void multiplyRow(int row, Fraction multiplier) throws Exception {
		fixHistory();
		for(int c = 0; c < COLUMNS; c++) {
			Fraction multiplicand = Fraction.parseFraction(JLMatrix[row][c].getText());
			JLMatrix[row][c].setText(multiplicand.multiply(multiplier).toString());
		}
		addToHistory();
	}
	
	public void divideRow(int row, Fraction divisor) throws Exception {
		fixHistory();
		for(int c = 0; c < COLUMNS; c++) {
			Fraction dividend = Fraction.parseFraction(JLMatrix[row][c].getText());
	...
	public void switchRow(int row1, int row2) throws Exception {
		fixHistory();
		for(int c = 0; c < COLUMNS; c++) {
			Fraction newRow1Element = Fraction.parseFraction(JLMatrix[row2][c].getText());
			Fraction newRow2Element = Fraction.parseFraction(JLMatrix[row1][c].getText());
	...
	private class MatrixPanel extends JPanel {
		public MatrixPanel(Fraction[][] matrix) {
